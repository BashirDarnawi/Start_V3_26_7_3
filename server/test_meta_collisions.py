...
def _choices(*pairs, **extra) -> dict:
    return {"signedBy": "Owner", "signedAt": "2026-09-25", **extra,
            "choices": [{"adId": ad_id, "choice": choice} for ad_id, choice in pairs]}
...
def test_report_finds_studio_rows_by_name_and_linked_campaign_id():
    ...
    assert rows[named]["lastModified"] == int(_row(named)["last_modified"])
...
def test_rows_with_money_are_refused_never_deleted():
    ...
    document = _choices(*[(ad_id, mc.REMOVE) for ad_id in money_rows + [customer_only, clean]])
    with db_conn() as conn:
...
def test_collision_repair_reversible():
    ...
    document = _choices((remove_a, mc.REMOVE), (remove_b, mc.REMOVE), (keep, mc.KEEP))
    document["choices"][0]["lastModified"] = int(before[remove_a]["last_modified"])
...
def test_reverse_refuses_when_a_row_changed_after_the_repair():
    ...
    with db_conn() as conn:
        _, reversal = mc.apply_repair(conn, _choices((removed, mc.REMOVE), (kept, mc.KEEP)))
    with db_conn() as conn:
...
def test_dry_run_and_report_change_nothing():
    ...
    before = snapshot()
    with db_conn() as conn:
        plan = mc.plan_repair(conn, _choices((first, mc.REMOVE), (second, mc.REMOVE)))
...
def test_missing_changed_foreign_and_closed_month_rows_are_refused():
    ...
    close_id = "financial-close-2019-03"
    with db_conn() as conn:
        ...
        _insert(conn, "financialClosures", close_id, {"status": "closed", "period": "2019-03"})
    document = _choices((changed, mc.REMOVE), (plain, mc.REMOVE), (removed_before, mc.REMOVE),
                        (f"ad_col_{TAG}_missing", mc.KEEP), (closed, mc.REMOVE))
    document["choices"][0]["lastModified"] = int(_row(changed)["last_modified"]) - 1
    try:
        with db_conn() as conn:
            result, reversal = mc.apply_repair(conn, document)
    finally:
...
@pytest.mark.parametrize("document, message", [
    ([], "JSON object"),
    ({"choices": [{"adId": "ad_1", "choice": mc.REMOVE}]}, "signedBy"),
    ({"signedBy": "Owner", "signedAt": "2026-09-25", "choices": []}, "no choices"),
    (_choices(("ad_1", "delete")), "choice must be"),
    (_choices(("ad_1", mc.REMOVE), ("ad_1", mc.KEEP)), "listed twice"),
    (_choices(("bad id!", mc.REMOVE)), "not a record id"),
    ({**_choices(("ad_1", mc.REMOVE)), "choices": [{"adId": "ad_1", "choice": mc.REMOVE, "lastModified": "x"}]},
     "lastModified"),
])
...
def test_command_line_dry_run_apply_and_reverse(tmp_path):
    ...
    assert {row["adId"] for row in json.loads(report.stdout)["rows"]} == {ad_id, paid_id}
    choices = tmp_path / "choices.json"
    choices.write_text(json.dumps(_choices((ad_id, mc.REMOVE), (paid_id, mc.REMOVE))), encoding="utf-8")
