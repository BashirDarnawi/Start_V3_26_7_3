...
function shellMoreTiles() {
  ...
  const tiles = [
    ...
    { id: 'ads-studio', icon: 'rocket', color: 'from-blue-600 to-cyan-500', name: isAr ? 'استوديو الإعلانات' : 'Ads Studio', sub: isAr ? 'منشورات وردود تلقائية' : 'Posts & auto-replies', allowed: userCanAccessView(state.currentUser, 'ads-studio') }
  ];
      ...
      <div class="shell-more-grid grid grid-cols-2 sm:grid-cols-3 gap-3">
        ${tiles.map(tile => `
          <button type="button" onclick="navigateTo('${tile.id}')" class="hub-card hub-tile w-full p-4 text-start touch-target">
            ...
            <span class="shell-tile-name mt-3 block text-sm font-bold text-slate-900 dark:text-white">${tile.name}</span>
...
function renderManagerHomeHero(receipts, ads, canViewFinancials) {
  ...
  return `
    <section class="manager-home-hero workspace-home-overview" data-manager-home-hero>
      <div class="workspace-home-primary">
      <button type="button" onclick="navigateTo('receipts')" class="hub-hero relative overflow-hidden w-full rounded-3xl p-5 text-start text-white touch-target">
        ...
        <span class="relative block">
          ...
          <span class="shell-hero-value block mt-1 text-3xl font-black tracking-tight" dir="ltr">${canViewFinancials ? shellEsc(shellLyd(collectedLyd)) : receiptsThisMonth}</span>
      ...
      <div class="shell-quick-actions grid gap-2" style="grid-template-columns: repeat(${quick.length}, minmax(0, 1fr));">
        ${quick.map(q => `<button type="button" onclick="${q.onclick}" class="shell-quick-action touch-target flex flex-col items-center justify-center gap-1.5 p-3 text-center"><span class="shell-quick-icon w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 flex items-center justify-center"><i data-lucide="${q.icon}" class="w-5 h-5"></i></span><span class="text-[11px] font-bold text-slate-700 dark:text-slate-200">${q.label}</span></button>`).join('')}
      </div>
      <div class="shell-home-kpis grid grid-cols-3 gap-2">
      ...
      <div class="workspace-home-activity">
        ...
        ${recent.length ? `<div class="space-y-2">${recent.map(r => {
          ...
          return `<button type="button" onclick="openReceiptFromHome('${shellEsc(r.id)}')" class="shell-home-recent hub-card hub-row w-full flex items-center gap-3 p-3 text-start touch-target">
...
function shellRowIsOpen(kind, id, defaultOpen = false) {
  const key = shellRowKey(kind, id);
  return _shellExpanded.has(key) || (defaultOpen && !_shellCollapsed.has(key));
}

function shellRowDomId(kind, id, part) {
  // Encode every code point, including punctuation, so imported ids cannot
  // collide after sanitizing and cannot become HTML or selector syntax.
  const encode = value => Array.from(String(value || ''), char => char.codePointAt(0).toString(16)).join('-');
  return `shell-${encode(kind)}-${encode(id)}-${part}`;
}

function shellToggleRow(kind, id) {
  const key = shellRowKey(kind, id);
  const buttonId = shellRowDomId(kind, id, 'toggle');
  const button = document.getElementById(buttonId);
  const open = button ? button.getAttribute('aria-expanded') === 'true' : shellRowIsOpen(kind, id);
  if (open) { _shellExpanded.delete(key); _shellCollapsed.add(key); }
  else { _shellExpanded.add(key); _shellCollapsed.delete(key); }
  render();
  // Rendering replaces the button; preserve keyboard focus without jumping
  // the page back to its top or opening an unrelated record.
...
function shellSummaryButton({ kind, id, avatar, title, sub, trailing = '', open = false }) {
  ...
  return `
      <button type="button" id="${shellRowDomId(kind, id, 'toggle')}" data-shell-kind="${safeKind}" data-shell-id="${safeId}" onclick="shellToggleRow(this.dataset.shellKind, this.dataset.shellId)" aria-expanded="${open ? 'true' : 'false'}" aria-controls="${shellRowDomId(kind, id, 'details')}" class="shell-summary-button ${avatar ? 'has-avatar' : ''} w-full p-3.5 text-start touch-target">
        ${avatar ? `<span class="shell-summary-avatar" aria-hidden="true">${avatar}</span>` : ''}
        <span class="shell-summary-main min-w-0">
          <span class="shell-summary-title block text-[15px] font-bold text-slate-900 dark:text-white">${title}</span>
          ${sub ? `<span class="shell-summary-sub block text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">${sub}</span>` : ''}
        </span>
        ${trailing ? `<span class="shell-summary-trailing">${trailing}</span>` : ''}
        <span class="shell-summary-disclosure"><span>${open ? (isAr ? 'إخفاء' : 'Hide') : (isAr ? 'التفاصيل' : 'Details')}</span><i data-lucide="${open ? 'chevron-up' : 'chevron-down'}" class="w-4 h-4 flex-shrink-0" aria-hidden="true"></i></span>
      </button>`;
}

function shellDirectoryFact(label, value, tone = '') {
  return `<div class="workspace-record-fact ${tone}"><dt>${shellEsc(label)}</dt><dd>${value}</dd></div>`;
}

// Only fixed application handlers are accepted. Record IDs remain data, never
// executable JavaScript, including imported IDs containing quotes or Unicode.
function shellRecordAction(action, id, label, icon = 'arrow-up-right') {
  const handlers = {
    editCustomer: 'editCustomer(this.dataset.recordId)',
    customerCoverage: 'openCustomerCompanyDebtCoverage(this.dataset.recordId, this)',
    receiptAds: 'openReceiptAds(this.dataset.recordId)',
    receiptPhotos: 'openReceiptPhotoViewer(this.dataset.recordId, 0)',
    editReceipt: 'editReceipt(this.dataset.recordId)',
    receiptCoverage: 'openCompanyDebtCoverageModal(this.dataset.recordId, this)',
    editPage: 'editPage(this.dataset.recordId)',
    editUser: 'editUser(this.dataset.recordId)'
  };
  if (!handlers[action]) return '';
  return `<button type="button" class="workspace-record-action" data-record-action="${action}" data-record-id="${shellEsc(id)}" onclick="${handlers[action]}"><i data-lucide="${shellEsc(icon)}" aria-hidden="true"></i><span>${shellEsc(label)}</span></button>`;
}

function shellListRow({ kind, id, avatar, title, sub, trailing = '', facts = '', actions = '', card = '', open = false, accent = '' }) {
  const safeKind = shellEsc(kind);
  const safeId = shellEsc(id);
  return `
    <article class="hub-card shell-row workspace-record ${open ? 'is-open' : ''}" data-shell-row="${safeKind}" data-shell-row-id="${safeId}"${accent ? ` style="border-inline-start:4px solid ${shellEsc(accent)}"` : ''}>
      ${shellSummaryButton({ kind, id, avatar, title, sub, trailing, open })}
      ${!open && facts ? `<dl class="workspace-record-facts">${facts}</dl>` : ''}
      ${!open && actions ? `<footer class="workspace-record-actions">${actions}</footer>` : ''}
      ${card ? `<div id="${shellRowDomId(kind, id, 'details')}" class="shell-row-body" aria-labelledby="${shellRowDomId(kind, id, 'toggle')}"${open ? '' : ' hidden'}>${card}</div>` : ''}
    </article>`;
}
...
function shellTableDetailAttrs(kind, id) {
  return `id="${shellRowDomId(kind, id, 'details')}" data-shell-detail="${shellEsc(kind)}" data-shell-detail-id="${shellEsc(id)}"`;
}

function shellAdMedia(ad, isAr) {
  if (typeof renderAdPrimaryThumbnail !== 'function') return '';
  const privatePhoto = can('ads', 'viewPhotos') && getAdPhotoCount(ad) > 0;
  const metaSource = typeof metaAdThumbnailSrc === 'function' ? metaAdThumbnailSrc(ad) : '';
  // Delegate uploads and their protected endpoint to the existing renderer.
  // Do not attach arbitrary legacy URL schemes to a new image surface.
  if (!privatePhoto && metaSource && !isSafeReceiptPhotoSource(metaSource)) return '';
  return renderAdPrimaryThumbnail(ad, isAr);
}

function shellAdBudgetPreview(ad, isAr) {
  if (!ad?.metaAdId || typeof renderMetaAdBudgetSummary !== 'function') return '';
  const total = metaAdsPlannedTotalMinor(ad);
  const remaining = metaAdsTotalRemainingMinor(ad);
  const knownMinor = value => value != null && typeof value !== 'boolean'
    && String(value).trim() !== '' && Number.isFinite(Number(value)) && Number(value) >= 0;
  const hasRemainingField = Object.prototype.hasOwnProperty.call(ad, 'metaTotalRemainingBudgetMinor');
  const knownProgress = hasRemainingField
    ? knownMinor(ad.metaTotalRemainingBudgetMinor)
    : knownMinor(ad.metaSpendMinor);
  // This ratio is presentation only. Amounts, currency and open-ended budget
  // rules remain in the same authoritative renderer used by the full card.
  const percent = knownProgress && Number.isFinite(total) && total > 0 && Number.isFinite(remaining)
    ? Math.max(0, Math.min(100, (1 - remaining / total) * 100)) : null;
  return `<div class="shell-ad-budget" data-shell-ad-budget="${shellEsc(ad.id)}">
    ${renderMetaAdBudgetSummary(ad, isAr)}
    ${percent !== null ? `<div class="shell-budget-track" role="progressbar" aria-label="${isAr ? 'المستخدم من ميزانية Meta' : 'Meta budget used'}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${percent.toFixed(1)}"><span style="width:${percent.toFixed(1)}%"></span></div>` : ''}
  </div>`;
}
...
function shellAdSummaryRow(ad, meta = {}) {
  ...
  if (ad?.metaAdName && ad.metaAdName !== name) subParts.push(`<span class="shell-ad-campaign">${shellEsc(ad.metaAdName)}</span>`);
  const pageName = meta.adPage?.name || ad?.metaPageName;
  if (pageName && pageName !== name) subParts.push(`${isAr ? 'الصفحة' : 'Page'}: ${shellEsc(pageName)}`);
  const accountName = ad?.metaAdAccountName || ad?.metaAdAccountId;
  if (accountName) subParts.push(`${isAr ? 'الحساب' : 'Account'}: ${shellEsc(accountName)}`);
  ...
  const media = shellAdMedia(ad, isAr);
  return shellTableSummaryRow('ads', id, {
    avatar: media ? '' : shellAvatar(shellInitial(name), tone),
    title: shellEsc(name),
    sub: subParts.join(' · '),
    trailing: `${amount}<span class="shell-summary-pills">${pills}</span>`,
    media,
    extra: shellAdBudgetPreview(ad, isAr)
  }, 10);
}
...
function shellReceiptRow(receipt, customer, card, meta = {}) {
  ...
  const trailing = pill;
  const facts = meta.destroyed ? '' :
    shellDirectoryFact(meta.hasCustomerDebt ? (isAr ? 'دين العميل' : 'Customer owes') : (isAr ? 'قيمة الوصل' : 'Receipt value'), `<span dir="ltr">${shellEsc(shellLyd(amountLyd))}</span>`, meta.hasCustomerDebt ? 'is-debt' : '')
    + shellDirectoryFact(meta.hasCustomerDebt ? (isAr ? 'دين مرتبط بإعلانات' : 'Debt linked to ads') : (isAr ? 'المستخدم للإعلانات' : 'Used for ads'), `<span dir="ltr">${shellEsc(shellUsd(meta.displayedUsedUSD || 0))}</span>`)
    + shellDirectoryFact(meta.hasCustomerDebt ? (isAr ? 'دين غير مخصص' : 'Unassigned debt') : (isAr ? 'الرصيد المتاح' : 'Available balance'), `<span dir="ltr">${shellEsc(shellUsd(meta.displayedRemainingUSD || 0))}</span>`, meta.hasCustomerDebt ? 'is-debt' : 'is-credit');
  let actions = '';
  if (!meta.destroyed) {
    if (meta.canSeeReceiptAds) actions += shellRecordAction('receiptAds', id, `${isAr ? 'الإعلانات' : 'Ads'} ${Number(meta.linkedAdCount) || 0}`, 'megaphone');
    if (meta.receiptPhotoCount > 0) actions += shellRecordAction('receiptPhotos', id, `${isAr ? 'الصور' : 'Photos'} ${meta.receiptPhotoCount}`, 'images');
    if (meta.canEditThisReceipt) actions += shellRecordAction('editReceipt', id, isAr ? 'تعديل' : 'Edit', 'pencil');
    if (meta.canCoverWithCompanyFunds && isCurrentUserAdmin()) actions += shellRecordAction('receiptCoverage', id, isAr ? 'سداد من أموال الشركة' : 'Pay debt from company funds', 'landmark');
  }
  const subParts = [];
...
function shellCustomerRow(customer, stats, card, meta = {}) {
  ...
  const facts = meta.canSeeBalance && stats
    ? shellDirectoryFact(isAr ? 'إجمالي المدفوع' : 'Total paid', `<span dir="ltr">${shellEsc(shellLyd(stats.totalPaidLYD || 0))}</span>`)
      + shellDirectoryFact(isAr ? 'تكلفة الإعلانات' : 'Ad charges', `<span dir="ltr">${shellEsc(shellLyd(stats.totalSpentLYD || 0))}</span>`)
    : '';
  let actions = (meta.linkedReceiptsButton || '') + (meta.linkedPagesButton || '');
  if (meta.canEditThisCustomer) actions += shellRecordAction('editCustomer', id, isAr ? 'تعديل' : 'Edit', 'pencil');
  if (meta.canSeeBalance && meta.coverableDebtUSD > 0.005 && isCurrentUserAdmin()) actions += shellRecordAction('customerCoverage', id, isAr ? 'سداد من أموال الشركة' : 'Pay debt from company funds', 'landmark');
  return shellListRow({
    ...
    sub: `${meta.displayNum ? `#${meta.displayNum} · ` : ''}${sub}`, trailing, facts, actions
  });
}
...
function shellPageRow(page, card, meta = {}) {
  ...
  const facts = shellDirectoryFact(isAr ? 'المالك' : 'Owner', shellEsc(owners.map(owner => owner.name || '').join('، ') || (isAr ? 'غير معين' : 'Not assigned')))
    + (meta.canSeePageAds ? shellDirectoryFact(isAr ? 'الإعلانات المرتبطة' : 'Linked ads', String(Number(meta.pageStats?.totalAds) || 0)) : '')
    + (meta.canSeePageFinancials && meta.pageStats ? shellDirectoryFact(isAr ? 'تكلفة الإعلانات' : 'Ad charges', `<span dir="ltr">${shellEsc(shellUsd(meta.pageStats.totalSpendUSD || 0))}</span>`) : '');
  const actions = can('pages', 'edit') ? shellRecordAction('editPage', id, meta.needsPageOwner ? (isAr ? 'تعيين المالك' : 'Assign owner') : (isAr ? 'تعديل الصفحة' : 'Edit page'), meta.needsPageOwner ? 'user-plus' : 'pencil') : '';
  return shellListRow({
    kind: 'pages', id, card,
    ...
    trailing: pills.join(''), facts, actions
  });
}
...
function shellUserRow(user, card, meta = {}) {
  ...
  return shellListRow({
    ...
    trailing: rolePill,
    actions: meta.canEditThisUser ? shellRecordAction('editUser', id, isAr ? 'تعديل الملف' : 'Edit profile', 'pencil') : ''
  });
}
