...
## Round 14 (2026-09-19, night): permission matrix, Ads Studio journey, money readers — last round of the loop

Four hunters: a permission-matrix auditor (every route and client gate against every grant), an Ads Studio customer-journey walk (sign-up to account deletion, phone and laptop), a money-readers cross-check (27 customer histories executed through the real client functions and their server twins), and a Meta sync hunter (no report by release time). Tests: `server/test_deep_scan_round14.py`; client checks in `scripts/test-review-regressions.js`. The owner asked for the scan loop to stop after this round.

### Fixed

**Permissions**
- A delegated user manager could take over a stronger colleague in three steps: change their role to Delivery, reset their password (Delivery targets are exempt from the takeover guard because resetting a driver's password is the everyday case), change the role back. Changing the role of a user who holds a grant you lack is now refused, in both directions. A held full grant covers its "own" variant and a driver's own-scope grants do not count, so an office manager still re-roles a normal driver.
- `POST /api/audit/cleanup` (the "clear old logs" button) deleted every row older than the cutoff, including the month-close, import, restore and company-coverage rows the retention job keeps forever. It now keeps the same protected set.
- `ads.stopAd` and `receipts.transfer` were not record-scoped: an employee with `viewOwn` could stop any ad or move dollars off any paid receipt by id (ids appear in audit rows). Both routes now require that the actor can see the record (and the delivery scope on transfers).
- A driver holding an extra grant (pages, clothes, campaigns) got no sync watermark for it and reloaded the whole collection every tick; other grants now use the generic rule.
- Every staff member with `analytics.view` landed on the admin-only Control Center and saw "No access"; the landing chooser skips admin-only views for staff. The user form disables the role select and the password field for a target who holds a grant the editor lacks (the server refused those anyway).

**Ads Studio**
- Staff "Stop & refund" on a LAUNCHED campaign defaulted to refunding the whole budget (nothing records Meta spend, so "unspent" was always everything): every routine close of a finished ad returned money Albayan had already paid Meta. Closing a launched campaign now requires the unspent amount explicitly (0 closes it without a refund); the client pre-fills 0.00 and labels the button "Close campaign".
- A reviewer could not stop their own approved campaign from the UI (the client always sent a refund amount, which the server refuses for one's own campaign); own campaigns follow the customer rule on the client too. The review badge no longer counts the reviewer's own submitted campaign.
- In the standalone studio (`/studio`) the paywall's "Charge wallet" looped back to the paywall, and the studio form could only request USD while plans are priced in LYD, so a lapsed customer could never renew alone. The form offers USD or LYD and the paywall button reaches it, also under the activate card.
- The client judged "today" by the phone's timezone while the server uses the Libya day (a stop offered at 22:00 abroad was refused); a changed plan price reloads the sheet instead of failing until it is reopened; campaign refusals reach Arabic customers in Arabic; approving a request whose end date passed is a clear 409 instead of "startDate cannot be in the past"; a draft-save conflict reloads the form from the other device's version (it conflicted forever until the builder was closed); the storage-quota hint names an action the customer can take.

**Money readers**
- The customer card ignored the cash a driver collected on an UNDERPAID delivery whose ad still holds its due row: a $100 ad with $70 collected read as -$100 instead of -$30 (hero "Owed", Collect and reminders inherited it). Driver-collected cash is now customer cash, the rule liquidity already used.
- Company coverage recorded on a receipt but never attached to an ad row (coverage given before the ad existed) was charged to the customer forever on the card while every server reader credited it; the card credits it once, exactly like the server coverage state.
- The Collect screen printed the LYD balance under a dollar sign and selected debtors by the LYD balance; it now selects and prints the USD balance. Analytics volume/collected/pending, the hero "Collected this month" and the month-close snapshot count driver-collected cash as collected (they did not add up). The outstanding/coverable helpers return 0 for paid or closed receipts (their callers filtered; now they are safe on their own).
- Reverted after the guards: counting a rowless driver ad's remainder against its delivery receipt. `test-permissions` pins the rule that such an ad is a separate debt while the receipt is unpaid (the server's `_financial_ad_committed` says the same); the double reading on the card stays open below.

### Review of these fixes (same night)
An adversarial reviewer verified the diff against the code and the pinned suites; corrected: the role guard matched grants literally and would have locked every non-admin manager out of re-roling a template driver (`customers.view` did not cover `customers.viewOwn`, nobody in the office holds `deliveries.complete`); the draft-conflict change bumped only the baseline while the form kept the stale text, which would have let the next Save overwrite the other device's version (the form is now reloaded like the modal path); the studio "charge wallet" jump selected a tab id that does not exist; the Collect screen selected debtors by the LYD balance but printed the USD one; the campaign card, the stop prompt and the server now use one "launched" predicate (publish status or Meta id).

### Verified sound (no change)
- Every list/get/delta/bootstrap route applies view/viewOwn with the creator; delivery reach (assignment scope on ads/receipts, referenced customers, settle/unsettle/check-stuck); contact projection by `viewContacts`; self role change, self delete, last admin, non-admin granting Admin all refused; wallet/plan/import/restore/merge/coverage/backup routes admin-only; debug routes gated; the `_unheldGrant` client guard equals the server grant rule.
- Ads Studio money doors: one capture per submission cycle, orphan release on reject/changes/resubmit/archive/delete, refund refused when a release exists and vice versa, holds respected by every debit, idempotent submit/review/stop/publish with 409 reconcile, self-review 403, lapsed customers can still stop and be refunded, media limits, account deletion refused with live campaigns.
- Money readers agree with the server on all 27 histories except the product decisions below; indexed and direct customer stats are identical everywhere; transfers are neutral across card, liquidity, analytics and month close; carried balances stay out of every sales total.

### Still open for the owner
- Seven permissions are decorative (no server or client site honours them): `ads.changeStatus`, `pages.linkCustomers`, `deliveries.complete`, `users.viewActivity`, `analytics.export`, `customers.export`, `settings.edit`; `ads.assignDelivery` only feeds the name directory. Presets over-promise them. Remove them from the matrix or wire them.
- Clothes routes check the subscription before the permission (a permission-less employee is told to subscribe), and staff "view all" grants on clothes collections hit the paywall because there is no staff exemption like the Ads Studio reviewer one.
- A finished Ads Studio campaign has no customer-side exit: archive is refused while the capture is open, so a customer waits for staff to close it. Storage quota counts finished campaigns until then.
- A driver-canceled delivery is customer debt on the card and "pending/unpaid" for hero, analytics and the month close, but "no debt" for the server canonical reader and the coverage route: choose one rule (round 13).
- A rowless driver ad and its unpaid delivery receipt read as two debts on the card (pinned rule); a written-off ad is debt on the card and "pending" in analytics but nothing for the month close; a canceled paid receipt awaiting refund shows nowhere per customer; two pot readers disagree on an underpaid receipt (display only; the server enforces).
- Meta sync hunter: it had not reported by release time; if a report arrives it is filed here without code changes (the loop is stopped).
