...
| P4-01 + P4-02 + P4-03 + P4-05 | Done (server) | stage 13 | Rules reference pages (`pageRefs`, "page removed" stays editable), reply log route with per-action saves (a restart never resends a sent action), page health states/reasons + subscribe on link + daily budgeted check, capability gates from the setting (armed after the first admin save) |
| P4-04 + P4-09 | Done (server) | stage 13 | Per-page parks on the page lane; Instagram polling source in the jobs loop (5-min, cursors, budget, exactly-once with the webhook; only when `igPublicReply` = poll) |
| P3-06a + P3-06d | Done (server) | stage 13 | Settle gates in the staff stop (delivering / before the final read / cap = paid − Meta spend; never-delivered full return; they survive an unlink), admin settle override with reason + `meta_overspend` alert (D27) |
