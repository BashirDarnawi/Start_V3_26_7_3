...
| P2-08 | Done | stages 10–17 | v2 styles in `assets/ads-workspace.css` for the frame and every screen (320–820 px, light/dark, RTL); a check keeps every block closed |
...
| P3-05 | Done | stage 12 | Activity feed + inbox (`GET /api/studio/activity`, seen marker); events at review, link/live, stop, ticket answered |
| P3-07 + P3-13 | Done | stage 12 | Help tickets (`/api/studio/tickets`, `/api/studio/staff/tickets`): numbers unique under 50 parallel opens (PostgreSQL), caps, 7-day reopen, reviewer 404 on payment/account tickets. `services.help` is off until the owner switches it on |
| P3-10 + P3-11 | Done | stage 12 | Stop request opens an urgent ticket on the same transaction, auto-resolves when the ad stops, overdue alert; staff contact link only with consent, audited |
...
| P3-17 | Done | stage 12 | Staff pulse (`GET /api/studio/staff/pulse`): counts only; admins also see payments waiting |
...
| P4-01 + P4-02 + P4-03 + P4-05 | Done | stage 13 | Rules reference pages (`pageRefs`, "page removed" stays editable), reply log route with per-action saves (a restart never resends a sent action), page health states/reasons + subscribe on link + daily budgeted check, capability gates from the setting (armed after the first admin save) |
| P4-04 + P4-09 | Done | stage 13 | Per-page parks on the page lane; Instagram polling source in the jobs loop (5-min, cursors, budget, exactly-once with the webhook; only when `igPublicReply` = poll) |
| P3-06a + P3-06d | Done | stage 13 | Settle gates in the staff stop (delivering / before the final read / cap = paid − Meta spend; never-delivered full return; they survive an unlink), admin settle override with reason + `meta_overspend` alert (D27) |
...
| P5-01 + P5-04 | Done | stage 13 | TikTok service requests as tickets (≤3 open, states, bilingual notes, never "connected/automated"); anonymisation scrubs Social Studio rows and TikTok handles/notes |
...
| P4-06 + P4-07 + P5-03 | Done (client) | stage 14 | v2 Pages & replies (pages with health, rules with pageRefs and honest channel states, rule editor, reply log, posts), page-link request with the Instagram pre-check answers, seven bilingual help guides; classic Replies/Posts tabs hand over when the layout is v2 |
| P3-06b + P3-06c + P3-09 + P3-13 + P3-17 (client) | Done | stage 14 | v2 Team desk in its own bundle `studio-staff.js`: requests (reason picker), launch & link, settle (countdown to the final read, override), tickets, health, More (payments, alerts, diagnostics, collisions, **admin settings forms for every key**), pulse badge + title count |
| P5-02 + P3-04b (client) + P2-09 + P2-11 | Done | stage 14 | TikTok service request UI (help by hand, never "connected"), inbox pulse/badge, v2 results card, Android Back hook + login help line with the public contact (`GET /api/studio/public/contact`, +200 B startup), error-map completeness check |
| Bundle split | Done | stage 14 | Three studio bundles, each under 1 MiB: `studio.js` (~950 KB, customer screens), `studio-staff.js` (~212 KB, Team desk + health), `studio-pages.js` (~130 KB, Pages & replies); one named loader (15o0) fetches the extra two on demand |
| P3-23 | Done | stage 16 (server), stage 17 (desk button) | `POST /api/studio/admin/alerts/{id}/ack` (admin, audited, version-checked, replay no-op); acknowledged alerts leave the open list and the pulse count. Desk: an "Acknowledge" (تأكيد الاطلاع) button per open alert in More > Alerts (single flight, the row leaves the list, the pulse is read again; UNKNOWN_ALERT through the ONE error map) |
| P3-24 | Done | stage 16 (server), stage 17 (desk button) | `POST /api/studio/admin/integrity/scan` (admin, 1 per 10 min, audited): the daily money scan on demand with the same report and alert. Desk: "Scan money now" (افحص الأموال الآن) on the Diagnostics card (single flight, the counts in words, a 429 shows the wait) |
| P6-06 | Done | stage 18 | "Classic view / العرض القديم" link on the kicker line of the v2 header (customers and the Team desk): this tab draws the classic studio until "New studio / الاستوديو الجديد" in the classic header (drawn by 15c through one guarded hook, only while /me says v2). The choice lives in sessionStorage (this page's memory when storage is blocked), survives a reload of the tab, ends with it, never reaches another tab or the server. ST (sandbox) + E2E (frame spec) |
| P5-07 | Done | stage 18 | `/privacy#terms` linked from the v2 Account screen (between Privacy and Sign out), from the Help contact card (v2 screen and classic help tab) and from the login help line. The login line is drawn by 15r in the lazy studio bundle, so the link costs no startup bytes (script.js unchanged at 2,516,224 B, 358 B under the cap); 12-views keeps only its hook. ST + E2E |
| P3-25 | Done | stage 18 | "Test alert channel / اختبار قناة التنبيهات" in the Studio health screen: `POST /api/studio/admin/alert-channel/test` (single flight), the answer in words ({sent, configured}: sent / not set up (ALBAYAN_ALERT_WEBHOOK_URL) / set up but refused), a 429 shows the wait in minutes (Retry-After), ADMIN_ONLY in words. ST + E2E (staff spec) |
| P5-05 | Done | stage 18 | Every health item (token, webhook counters, jobs heartbeat, Meta lanes, FB replies, daily-budget requests, allowlist, minimum daily budget, page subscription, funds, spend drift, the page tests) carries a bilingual "What to do (runbook §x)" line with a link. The runbook is not served by the site (`/docs` is FastAPI's API docs in debug mode only), so the link opens the desk section or settings form that fixes or verifies the item (Diagnostics, Alerts, Reply channels, Budget limits, Settlement, Intake, Requests; the page items scroll to the Meta tests); in the classic review tab (no desk) the line names the place in words. The heartbeat and lanes come from `/api/studio/admin/diagnostics`, read at most every 10 minutes. ST + E2E |
| P2-10 | Done | stage 17 | `tests/e2e/studio-v2-money.spec.js`: draft → send → Reserved $70 → withdraw → restored (EN + AR); send → staff approve → In your ads → staff stop / customer stop before start → full return; the strip equals the wallet summary at every step |
| P3-25 + P5-05 + P5-07 + P6-06 | Done | stage 17 | Test-alert button, a "what to do" line on every health item, terms links (Account, Help, login line — 0 startup bytes), per-session Classic view link in both frames |
