...
| P3-23 | Done (server) | stage 16 | `POST /api/studio/admin/alerts/{id}/ack` (admin, audited, version-checked, replay no-op); acknowledged alerts leave the open list and the pulse count. Desk button pending |
| P3-24 | Done (server) | stage 16 | `POST /api/studio/admin/integrity/scan` (admin, 1 per 10 min, audited): the daily money scan on demand with the same report and alert. Desk button pending |
