...
- `kind` is one of: `reply_failure_burst`, `page_health_drop`, `meta_overspend`, `meta_drift` (post-settle Meta spend above the settled spend by more than $0.50; it replaces `post_settle_spend_drift`), `running_past_end`, `approval_interrupted`, `results_parked`, `studio_account_config`, `studio_funds_low`, `studio_account_inactive`, `studio_funds_unreadable`, `meta_connection_down`, `meta_token_expiring`, `replies_parked`, `instagram_comments_not_arriving`, `integrity_violation`, `jobs_heartbeat_late`, `review_overdue`, `stop_request_overdue`, `payment_confirm_overdue`, `storage_threshold`, `studio_core_collision`.
...
- **`intake`:** `open|paused`, `pausedMessageAr/En`, **`maxSubmissionsPerDay`** (D29; plan start value 5 — shipped default 500, effectively off, until the owner decides).
...
- **Lock table [VERIFIED for existing rows; planned rows marked].** This is the PostgreSQL order. On SQLite the same operations take the process locks in the order entity-patch lock → wallet lock (stop route `ad_campaign_actions.py:192-195`; delete `main.py:4940-4944`).
  ...
  | Any money-row insert whose `created_by` is the owner (the owner's withdraw, archive or stop; an admin approving their own request) | after the campaign row: an FK **KEY SHARE** lock on the owner's `users` row (PostgreSQL's foreign-key check of `entities.created_by`) | PostgreSQL FK rule |

  **Global order:** user rows (sorted) → `cpay:` key → one campaign row → other advisory keys (`rel:`, `stoprefund:`, `subscription`, `subpay:`, `payreq:`, transfer keys). Every operation takes a subset of this order and at most one campaign row. The capture does not lock the user row. This is safe because the campaign's budget stays inside "Reserved" until the Approved status is written; the ledger debit and the hold overlap briefly, which only *understates* Available. The submit's money check reads the holds first, then the balance: no hold can grow while it holds the owner's row, so a sibling approval committing between the two reads only lowers Available. **Proof:** PostgreSQL Barrier scenarios in P1-19, both orders of each pair, and "submit racing a full approval of a sibling".

  **The one exception to the order (last row):** the KEY SHARE lock conflicts with the owner row's FOR UPDATE, so a "campaign row → money insert" by the owner waits for a submit (or a plan purchase, transfer or charge confirm) holding that row; if that submit is itself waiting for the same campaign row, PostgreSQL finds the deadlock and cancels one side. Money stays consistent (each side is one transaction; the cancelled one rolls back whole) and that side answers an error the user simply sends again.
- **Concurrency rules:**
