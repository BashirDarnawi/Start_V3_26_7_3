...
- `adLimits`: `minTotalMinorUSD`, `maxTotalMinorUSD`, `minPerDayMinorUSD`, `maxDays`; **`p1CutoverAt`** (set by the P1 release, P1-18).
- **`studioAccounts`:** `[{accountId, studioSince}]` (must match the env list; mismatch → fail closed).
- `settlement`: **`spendDelayHours` (default 48, D28)**, **`neverDeliveredImmediate`** (default true), **`driftWatchDays`** (28).
- **`serviceHours`:** `timezone` (`Africa/Tripoli`), `workingDays` (default Sun–Thu [ASSUMPTION: owner confirms in D11]), `open`/`close` (default 09:00–17:00), `overrides[]` (date ranges with other hours, e.g. Ramadan), `holidays[]` (dates), **`onDutyUntil`** (e.g. 23:00 for urgent stops).
- `contact`: public WhatsApp and phone; **`urgentWhatsapp`** (the on-duty number, shown only in the after-hours stop sheet).
- **`targets`** (D11): review, ticket first response, stop request, settlement (measured from `settleReadDueAt`), TikTok, **payment confirmation**.
