...
- **لا يوجد اليوم زر «أعد الفحص الآن».** الفحص المالي للاستوديو يعمل فقط في حلقة المهام اليومية؛ العنوان `GET /api/admin/data-integrity` هو فحص المنصة (العلاقات والتكرارات في `server/data_integrity.py`) ولا يتضمن فحوص أموال الاستوديو. `[developer]` يشغّل `run_daily_money_check` يدوياً عند الحاجة، وطلب زر لذلك مسجَّل كنقص.
  **There is no "scan again now" button today.** The studio money scan runs only in the daily jobs loop; `GET /api/admin/data-integrity` is the platform scan and does NOT include the studio money checks. `[developer]` runs `run_daily_money_check` by hand when needed; a button for it is an open gap.
...
تصل الثلاثة إلى القناة. مرة واحدة لكل حساب ونوع في اليوم. **لا يوجد زر «تم الاطلاع» في البيان** (`acknowledgedAt` حقل لا يكتبه أي مسار اليوم): يتوقف التنبيه عندما ينجح الفحص التالي.
All three reach the channel, once per account, kind and day. **There is no acknowledge button in Albayan** (no route writes `acknowledgedAt` today): the alert stops when the next check passes.
