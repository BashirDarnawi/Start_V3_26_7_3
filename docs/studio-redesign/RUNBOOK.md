...
2. **الطابور** / **The queue:** مكتب المراجعة: كم طلباً بانتظار المراجعة؟ هل يوجد «عاجل: طلب إيقاف» في قسم التذاكر؟ (العدّاد الحي يقرأ `GET /api/studio/staff/pulse`: `waitingReview`, `stopRequests`, `openTickets`, `paymentsWaiting`, `alerts`.) / The review tab: requests waiting? any "Urgent: stop request" in the tickets section? (The live counter reads `/api/studio/staff/pulse`.)
...
تصل الثلاثة إلى القناة. مرة واحدة لكل حساب ونوع في اليوم. **تأكيد الاطلاع:** يستطيع المدير تأكيد الاطلاع على تنبيه عبر `POST /api/studio/admin/alerts/{id}/ack` فيختفي من قائمة التنبيهات المفتوحة (تبقى في `?status=all`)؛ زر «تم الاطلاع» في مكتب الفريق قيد الإضافة (P3-23). ويتوقف التنبيه أيضاً عندما ينجح الفحص التالي.
All three reach the channel, once per account, kind and day. **Acknowledging:** an admin can acknowledge an alert with `POST /api/studio/admin/alerts/{id}/ack`; it leaves the open list (still in `?status=all`); the desk button is being added (P3-23 client). The alert also stops when the next check passes.
...
- يسري عند تحميل الصفحة التالي (الشاشة تقرأ `/api/studio/me` عند الفتح). لا يوجد رابط «العرض الكلاسيكي» للجلسة الواحدة (§12.2(e)) في الكود بعد. / Effective on the next page load. There is no per-session "Classic view" link in the code yet.
