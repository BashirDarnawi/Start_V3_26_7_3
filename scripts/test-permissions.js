...
for (const lazyBundle of ['studio.js', 'studio-staff.js', 'studio-pages.js', 'clothes.js', 'admin-tools.js']) {
  vm.runInContext(
...
check('server money/subscription calls use dedicated transactional endpoints', () => {
  ...
  const built = fs.readFileSync(SCRIPT, 'utf8')
    ...
    + fs.readFileSync(path.join(__dirname, '..', 'studio-staff.js'), 'utf8')
    + fs.readFileSync(path.join(__dirname, '..', 'studio-pages.js'), 'utf8')
